...
class conventioncredit(models.Model):
    _name = 'conventioncredit'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = u'conventioncredit'
    ...
    def create(self, vals):
        ...
        return super(conventioncredit, self).create(vals)
    ...
    def write(self, vals):
        ...
        if 'penalite' in vals:
            vals['penalite'] *= 0.01
        return super(conventioncredit, self).write(vals)
    ...
    # avenant
    type_conventioncredit = fields.Selection([('conventioncredit', 'conventioncredit'), ('avenant', 'Avenant')], string='Type', default='conventioncredit')
    num_avenantcn = fields.Char(u'Numéro avenant', readonly=True)
    parent_id = fields.Many2one('conventioncredit', string='Convention origine', readonly=True)
    ...
    avenant_ids = fields.One2many('conventioncredit', 'parent_id', string='Avenants')
    # New checkbox fields
    ...
    numero_conventioncredit = fields.Char(string="Numéro de conventioncredit")
 # DATE LIMITE
    ...
    def action_cloturer(self):
        ...
        return {
            ...
            'target': 'new',
            'context': {
                'default_conventioncredit_id': self.id,
    ...
    @api.model
    def create_avenant(self, conventioncredit_id, avenant_vals):

        # Get the convention record
        conventioncredit = self.browse(conventioncredit_id)

        # Calculate the new montant value

        new_montant = conventioncredit.montant + avenant_vals['montant']
        ...
        avenant = self.create({
            ...
            'type_conventioncredit': 'avenant',

            'parent_id': conventioncredit_id,
        ...
        conventioncredit.montant = new_montant
    ...
    def action_open_avenant_wizard(self):
        ...
        return {
            'name': _('Modifier la conventioncredit'),
            'type': 'ir.actions.act_window',
    ...
    def action_view_conventions(self):
        ...
        return {
            ...
            'name': 'conventioncredits',
            'view_mode': 'tree,form',
            'res_model': 'conventioncredits',
            'domain': [('type_conventioncredit', '=', 'conventioncredit')],  # Filter to show only conventions
        }
